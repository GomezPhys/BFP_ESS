...
Df <- read_excel("~/Percentages.xlsx", sheet = "VO2%")
...
lmModel5 = lmer(VO2_Perc ~ Condition + Sex + (1|ID),
                data=Df, REML=FALSE)
summary(lmModel5)
...
pwc5 <- Df %>%
  pairwise_t_test(VO2_Perc ~ Condition, paired = TRUE,
...
Df %>% cohens_d(VO2_Perc ~ Condition,
