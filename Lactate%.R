...
Df <- read_excel("~/Percentages.xlsx", sheet = "Lactate%")
...
lmModel5 = lmer(Lactate_PERC ~ Condition + Sex + (1|ID),
                data=Df, REML=FALSE)
summary(lmModel5)
...
pwc5 <- Df %>%
  pairwise_t_test(Lactate_PERC ~ Condition, paired = TRUE,
...
Df %>% cohens_d(Lactate_PERC ~ Condition,
                paired = TRUE, hedges.correction = TRUE)%>%
  kbl(caption = "Effect Size") %>%
  kable_classic(full_width = F, html_font = "Cambria")

table1(~ Lactate_PERC  | Condition,
